...
`INVALID` if, for `UNSPECIFIED` reasons, the controllerdetermine the count to be
...
`B2`, then the bandwidth can be calculated as follows. The frequency of the
...
[[BC_BMASK]]
=== Bandwidth allocation control (`bc_bw_alloc`)
...
`RCID=x` by the sum of the `Mweight` of all other contending `RCID`s. This ratio
`P` is as follows:
