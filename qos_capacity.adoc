...
block if the capacity block mask programmed for `RCID` and `AT` associated with
the request has the corresponding bit set to 1 in the capacity block mask. At
least one capacity block must be allocated using `cc_block_mask` when allocating
